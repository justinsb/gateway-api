...
var _ reconcile.Reconciler = &TLSRouteController{}

// TLSRouteController reconciles a TLSRoute object
...
func (r *TLSRouteController) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if !route.ObjectMeta.DeletionTimestamp.IsZero() {
		if err := r.Gateway.DeleteTLSRoute(ctx, r.client, route); err != nil {
	...
	if err := r.Gateway.UpdateTLSRoute(ctx, r.client, route); err != nil {
		return reconcile.Result{}, err
	}
...
func (r *TLSRouteController) SetupWithManager(mgr ctrl.Manager) error {
	// addon.Init()

	r.finalizer = "TLSRouteController"
	...
	err = c.Watch(source.Kind(mgr.GetCache(), &gatewayapi.TLSRoute{}, &handler.TypedEnqueueRequestForObject[*gatewayapi.TLSRoute]{}))
	if err != nil {
		return err
	}
